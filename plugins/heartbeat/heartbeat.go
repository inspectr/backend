...
func (x *Heartbeat) Start(e chan transistor.Event) error {
	...
	cron.NewCronJob(cron.ANY, cron.ANY, cron.ANY, cron.ANY, cron.ANY, 0, func(time.Time) {
		event := transistor.NewEvent(transistor.EventName("heartbeat"), transistor.GetAction("status"), plugins.HeartBeat{Tick: "minute"})
		x.events <- event
	})

	cron.NewCronJob(cron.ANY, cron.ANY, cron.ANY, cron.ANY, 0, 0, func(time.Time) {
		event := transistor.NewEvent(transistor.EventName("heartbeat"), transistor.GetAction("status"), plugins.HeartBeat{Tick: "hour"})
		x.events <- event
	})
